...
可选 `synthesize=True` 在最后加一步综合，融合各步结论为单一高质量输出。日常调研、审查、简单验证仍使用 7 个原子工具，只有重要决策才触发 delegate_chain。
...
```
AgentParliament/
├── server.py              # MCP 服务接口层，8 个工具 handler
├── prompts.py             # prompt 纯函数 + 记忆块注入
├── chain.py               # delegate_chain 编排 + 工具调度器
├── render.py              # 结果渲染 + 结构校验
...
> **delegate_chain** 是跨角色工具：当 code_developer、untangler 或 project_planner 面对需要完整链路深入思考的重要决策时，可用它将多个原子工具串成思考链。日常场景仍使用上表中的原子工具。
...
> 这套角色分工是可选的。你也可以在任意支持 MCP 的客户端里直接调用这 8 个工具，不依赖上述 Agent。
