...
def peer_review(
    ...
    if structured:
        instruction = (
            ...
            "只输出JSON数组本身，不要包裹markdown代码块，不要添加任何解释文字。\n"
            "数组每个元素是一个对象，包含以下字段：\n"
...
def test_audit(
    ...
    prompt = (
        ...
        f"{focus_line}"
        "请按以下结构输出：\n"
        "1.【已覆盖】现有测试已经覆盖了哪些场景（无现有测试时跳过此项）\n"
        "2.【缺失场景】按重要性列出尚未覆盖的逻辑分支、边界条件、异常路径：\n"
        "   🔴 关键缺口（核心逻辑或易错路径未测）\n"
        "   🟠 重要缺口（边界值、异常处理未测）\n"
        "   🟡 次要缺口（锦上添花的场景）\n"
        "3.【建议用例】针对缺失场景，给出具体的测试用例描述（输入、预期输出、验证点）\n"
        "4.【总体评价】当前测试充分度（充分/基本充分/不足），以及优先补充建议"
        f"{_READONLY_NOTICE}"
    )
